#ifndef ___DEVICE_CONTROL__H___
#define ___DEVICE_CONTROL__H___
///Author: CCIT WANGTIANYU
///date: 2014-7-8


#define DEVICECONTROLINTERFACE_API

/************************************************************************/
/*密码模块初始化                                                        */
/************************************************************************/
DEVICECONTROLINTERFACE_API int InitDeviceControlInterface();
...
[in]uisrcLen：明文数据长度
[in]derCert：用户加密证书数据
[in]derCertLen：用户加密证书数据长度
[in]keyIndex:服务器密码机密钥索引
[out]bEncryptedData：密文串数据
[out]uiEncryptedDataLen：密文串数据长度                                 */
/************************************************************************/
DEVICECONTROLINTERFACE_API int instructCrpRequest(unsigned char *src, unsigned int uisrcLen,
                                                  unsigned char *derCert,  unsigned int derCertLen, int keyIndex,
                                                  unsigned char *bEncryptedData, unsigned int *uiEncryptedDataLen);

/************************************************************************/
/*安全短信/指令解密
[in]bEncryptedData：密文数据
[in]uiEncryptedDataLen：密文数据长度  
[in]derCert：用户签名证书数据
[in]derCertLen：用户签名证书数据长度 
[in]keyIndex:服务器密码机密钥索引
[out]bOutData：解密后的明文数据
[out]uiOutDataLen：解密后的明文数据长度                                 */
/************************************************************************/
DEVICECONTROLINTERFACE_API int instructDecrpRequest(unsigned char *bEncryptedData, unsigned int uiEncryptedDataLen,
                                                  unsigned char * derCert,  unsigned int derCertLen, int keyIndex,
                                                  unsigned char *bOutData, unsigned int *uiOutDataLen);
...
/*密码模块反初始化                                                      */
/************************************************************************/
DEVICECONTROLINTERFACE_API int FinalizeDeviceControlInterface();

DEVICECONTROLINTERFACE_API unsigned char * Encry(unsigned char *src, unsigned int uisrcLen,
                                                  unsigned char *derCert,  unsigned int derCertLen, int keyIndex,
                                                  unsigned char *bEncryptedData);
...
#define SWR_BASE64ERR           (SWR_CARD_BASE + 0x03000021)	//BASE64转化错误
#define SWR_MEMORYERR           (SWR_CARD_BASE + 0x03000022)	//分配空间错误
#define SWR_NOTINIT             (SWR_CARD_BASE + 0x03000023)	//密码模块未初始化




#endif //___METH_ECC_ENGINE__H___